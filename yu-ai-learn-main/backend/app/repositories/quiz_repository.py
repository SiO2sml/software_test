    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                "WHERE qs.user_id = %s "
                "AND EXISTS (SELECT 1 FROM answer_records ar WHERE ar.quiz_id = qs.quiz_id)",
                (user_id,),
            )
...
async def get_user_quiz_list(user_id: int, page: int, page_size: int) -> tuple[list[dict], int]:
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            await cur.execute(
                "SELECT COUNT(*) FROM quiz_sessions qs "
                "INNER JOIN answer_records ar ON qs.quiz_id = ar.quiz_id "
                "WHERE qs.user_id = %s",
                (user_id,),
            )
            ...
            await cur.execute(
                ...
                "ar.accuracy, ar.total_questions, "
                "qs.created_at "
...
async def count_today_sessions(user_id: int) -> int:
    """统计用户当天（服务器本地日期）创建的出题会话数（同步出题接口的每日限额依据）。"""
    pool = get_mysql_pool()
    if pool is None:
        return 0
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT COUNT(*) FROM quiz_sessions WHERE user_id = %s AND created_at >= CURDATE()",
                (user_id,),
            )
            row = await cur.fetchone()
            return row[0] if row else 0
...
async def delete_quiz_session(quiz_id: str, user_id: int) -> bool:
    """删除一次尚无答题记录的出题会话（用于答题中途退出时清理误导性记录）。

    仅允许删除没有 answer_records 的会话，已完成的闯关记录不受影响。
    返回是否实际删除成功。
    """
    pool = get_mysql_pool()
    if pool is None:
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                "AND NOT EXISTS (SELECT 1 FROM answer_records WHERE quiz_id = %s)",
